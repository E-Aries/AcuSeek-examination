...
**Windows** — 右键 `install.ps1` → "以 PowerShell 运行"：
...
```bash
git clone https://gitee.com/aries-minnasang/AcuSeek.git /opt/AcuSeek
cd /opt/AcuSeek
...
```bash
# 1. 克隆项目
git clone https://gitee.com/aries-minnasang/AcuSeek.git
cd AcuSeek
...
# 5. 启动后端
cd backend && nohup python -m uvicorn main:app --host 0.0.0.0 --port 8000 &

# 6. 启动前端
cd .. && nohup npx vite --host &
```
...
系统默认使用 **SQLite**（零配置，开箱即用），也支持 MySQL / MariaDB / PostgreSQL。

### 默认配置（SQLite）

`backend/config.py` 中的默认连接：
...
> 也支持直接修改 `backend/config.py` 中的 `DATABASE_URL`，效果相同。

### seed.py 做了什么事

| 操作 | 说明 |
|------|------|
| 建表 | 自动创建 8 张表：users、questions、categories、exams、exam_papers、operation_logs、system_settings、notifications |
| 插数据 | 写入初始用户、示例题目和分类 |
...
![考试管理](screenshots/exam-management.png)
...
![考生考试界面](screenshots/candidate-exam.png)
...
![考生成绩查询](screenshots/candidate-scores.png)
