...
@router.post("")
def create_user(data: CreateUserSchema, request: Request, admin = Depends(get_current_user), db = Depends(get_db)):
    ...
    db.add(user)
    db.commit()
    log_action(db, admin.username, "创建用户", data.username, "", ip=request.client.host or "" if request.client else "")
    db.refresh(user)
    return {"id": user.id, "message": "创建成功"}

@router.put("/{uid}")
def update_user(uid: int, data: UpdateUserSchema, request: Request, admin = Depends(get_current_user), db = Depends(get_db)):
    ...
    log_action(db, admin.username, "修改用户", user.username, "", ip=request.client.host or "" if request.client else "")
    db.commit()
    return {"message": "更新成功"}
...
def delete_user(uid: int, request: Request, admin = Depends(get_current_user), db = Depends(get_db)):
    ...
    log_action(db, admin.username, "删除用户", user.username, "", ip=request.client.host or "" if request.client else "")
    db.delete(user)
    db.commit()
    return {"message": "删除成功"}
