...
def submit_answers(paper_id: int, data: AnswerSubmit, request: Request, user = Depends(get_current_user), db = Depends(get_db)):
    ...
    log_action(db, user.username, "提交试卷", f"试卷#{paper_id}", "", ip=request.client.host or "" if request.client else "")
    db.commit()
